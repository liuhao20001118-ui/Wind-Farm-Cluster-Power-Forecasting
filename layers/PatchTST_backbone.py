...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        # norm
        if self.revin:
            z = z.permute(0, 2, 1)  # (32,19,144) ==> (32,144,19)
            # Instance Normalization
            z = self.revin_layer(z, 'norm')  # Z-Score归一化，零均值单位标准差（使得当前序列符合正态分布）
            z = z.permute(0, 2, 1)  # (32,144,19) ==> (32,19,144)

        # do patching  TODO: 注意，这里仅对数据在时间步长维度进行了划分，并没有进行embedding
        if self.padding_patch == 'end':
            z = self.padding_patch_layer(z)  # 调用torch的函数进行尾部填充 (32,19,144)==>(32，19，152)
        # 先把当前序列长度144填充到合适划分的长度152，然后使用下面的代码进行划分patch  (32，19，152) ==> (32，19，18，16)
        z = z.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # z: [bs x nvars x patch_num x patch_len]
        z = z.permute(0, 1, 3, 2)  # (32,19,16,18)                        # z: [bs x nvars x patch_len x patch_num]
        ...
        z = self.head(z)      # z: [bs x nvars x target_window]  # (32,19,512,18) ==> (32,19,144)
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        n_vars = x.shape[1]  # 19
        # Input encoding
        x = x.permute(0, 1, 3, 2)    # x: [bs x nvars x patch_num x patch_len]  (32, 19, 18, 16)
        x = self.W_P(x)  # 线性映射 x: [bs x nvars x patch_num x d_model] 将patch_len=16映射到512的向量 这里相当于值嵌入
        # u： torch.Size([608, 18, 512]) 将批次数与变量数相乘合并
        u = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))  # u: [bs * nvars x patch_num x d_model]
        # 实现了论文图1(b)里面的 Projection + Position Embedding 也就是把位置信息放到输入里面，得到的结果作为编码器输入
        u = self.dropout(u + self.W_pos)  # u: [bs * nvars x patch_num x d_model]  torch.Size([608, 18, 512])

        # Encoder  z(608, 18, 512) ==> (32,19,18,512) ==> (32,19,512,18)
